...
int main(int, char *argv[]) {
  ...
  try {
    ...
    ofstream outFile("wumpus_location1.txt");

    if (!outFile.is_open()) {
      cerr << "Error: Could not create wumpus_location1.txt" << endl;
    } else {
      cout << "Inferred MAP trajectory saved to file wumpus_location1.txt" << endl;
